...
```
yuque-skill/
├── SKILL.md                       # 触发描述与使用入口(Claude会自动读这一份)
...
Openclaw 的 Skill 加载机制会扫描 `~/.openclaw/skills/` 与项目级 `.openclaw/skills/` 目录。任选其一:
...
把生成的 `yuque.skill` 放进 Claude Code 的 plugin marketplace 即可。

> Skill 加载完成后,Claude 启动时会在系统提示里看到 `yuque` 这个 skill 及其触发条件,无需额外注册。
...
把上面两行写入 `~/.bashrc` / `~/.zshrc` / Openclaw 启动脚本,Claude 调用脚本时即可自动读取。
...
安装并配好 Token 之后,直接用自然语言描述需求即可,Claude 会自动加载 SKILL.md 并选择合适的脚本。常见用法举例:
